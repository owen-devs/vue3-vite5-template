                                        ...
                                        if (isColor) {
                                            ...
                                            if (cssColor?.components) {
                                                if (
                                                    ...
                                                    `var(${cssVarName})` === cssColor.components[0]
                                                ) {
                                                    const values = themeValues.get(name)
                ...
                async getCSS(context) {
                    const { css } = await context.generator.generate(
                        ...
                        keys.map(
                            (key) =>
                                `${
                                    defaultThemeNames.includes(key) ? `${key}:` : ''
                                }${PRESET_THEME_RULE}:${key}:${Date.now()}`
                        ),
                        { preflights: false }
                    )
                                ...
                                keys
                                    .map(
                                        (key) =>
                                            `((.${key}) ((${
                                                selectors[key]
                                                    ? escapeStringRegexp(selectors[key])
                                                    : `.${key}`
                                            }[{|,])|(.${key}\\\\:)))`
                                    )
                                    .join('|'),
                            ...
                            const replacedLine = line.replace(regexStr, (matchStr, ...params) => {
                                ...
                                for (let i = 0; i < matchGroup.length / 5; i++) {
                                    ...
                                    if (matchGroup[startIndex + 3]) {
                                        ...
                                        replacement = `${selectors[key]} ${
                                            matchGroup[startIndex + 2]
                                        }`
                                    }
                                    break
                                }
